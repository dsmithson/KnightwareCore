            ...
            private bool InnerTryEnter(bool synchronous = false)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        else if (_parent._owningId != _oldId && _parent._owningId != AsyncId)
                        {
                            // Another thread currently owns the lock
                            // Note: We check both _oldId (normal flow) and current AsyncId 
                            // (handles ConfigureAwait(false) where AsyncLocal context may not flow)
                            return false;
                        }
                        else
                        {
